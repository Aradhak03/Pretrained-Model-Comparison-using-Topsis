...
cosine_similarity = data['Cosine_Similarity'].values
euclidean_distance = data['Euclidean_Distance'].values
manhattan_distance = data['Manhattan_Distance'].values
...
normalized_matrix = np.column_stack([
    ...
    1 - (euclidean_distance / np.max(euclidean_distance)),
    1 - (manhattan_distance / np.max(manhattan_distance))
])
